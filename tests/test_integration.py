...
class TestEndToEnd:
    ...
    async def test_full_pipeline_contradicted(self, mock_llm):
    ...
    async def test_climate_change_flags_hallucination(self, mock_llm, climate_change_dataset):
    ...
    async def test_renewable_energy_no_hallucination(self, mock_llm, renewable_energy_dataset):
...
class TestMockLLM:
    ...
    async def test_mock_llm_claim_extraction(self, mock_llm):
    ...
    async def test_mock_llm_verification_supported(self, mock_llm):
    ...
    async def test_mock_llm_verification_contradicted(self, mock_llm):
    ...
    async def test_mock_llm_call_count(self, mock_llm):
        """MockLLM should track call count."""
        assert mock_llm.call_count == 0
        await mock_llm.generate("test prompt")
        assert mock_llm.call_count == 1
        await mock_llm.generate("test prompt 2")
        assert mock_llm.call_count == 2

    async def test_mock_llm_climate_hallucination(self, mock_llm):
    ...
    async def test_mock_llm_renewable_statistic(self, mock_llm):
